  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (appwriteError.message.includes("already exists")) {
        setError("This email is already registered. Please go to Sign In.");
      } else {
        setError(appwriteError.message || "An error occurred during sign up.");
      }
    } finally {
      setIsLoading(false);
    }
      ...
      >
        ...
        <GlassCard glow className="p-8 bg-black/60 border-white/10 backdrop-blur-xl shadow-2xl">
          ...
          <div className="mt-8 text-center text-sm text-gray-400">
            ...
            <Link href="/" className="font-medium text-neon-purple hover:text-neon-cyan transition-colors">
              Sign In
            </Link>
          </div>
        </GlassCard>
